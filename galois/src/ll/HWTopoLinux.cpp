...
struct AutoLinuxPolicy {
  ...
  void generateHyperthreads(const std::vector<cpuinfo>& vals) {
    ...
    virtmap = dense;
#if 0
    std::vector<int> newdense;
    for (std::vector<int>::iterator ii = dense.begin(), ei = ht; ii < ei; ) {
      int phys = vals[*ii].physid;
      int skip;
      for (skip = 0; vals[*ii].physid == phys; skip++, ii++)
        newdense.push_back(*ii);
      for (; skip > 0; skip--, ht++)
        newdense.push_back(*ht);
    }
    virtmap = newdense;
#endif
  }
