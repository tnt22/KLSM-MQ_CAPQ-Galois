...
extern Galois::Statistic* WLEmptyWork;
extern Galois::Statistic* nEmpty;
extern Galois::Statistic* nOverall;
extern std::string wl;
...
struct LowDegreeHasher: public std::unary_function<GNode, unsigned long> {
  unsigned long operator()(const GNode& val) const {
    return (unsigned long) val;
  }
};

struct LowDegreeComparer: public std::binary_function<const GNode&, const GNode&, unsigned> {
  unsigned operator()(const GNode& x, const GNode& y) const {
    unsigned rx = std::distance(HighDegreeIndexer::indexgraph->edge_begin(x, Galois::MethodFlag::NONE),
                              HighDegreeIndexer::indexgraph->edge_end(y, Galois::MethodFlag::NONE));
    unsigned ry = std::distance(HighDegreeIndexer::indexgraph->edge_begin(x, Galois::MethodFlag::NONE),
                              HighDegreeIndexer::indexgraph->edge_end(y, Galois::MethodFlag::NONE));
    return rx > ry;
  }
};

struct LowDegreeNodeComparer: public std::binary_function<const GNode&, const GNode&, unsigned> {
  unsigned operator()(const GNode& x, const GNode& y) const {
    unsigned rx = std::distance(HighDegreeIndexer::indexgraph->edge_begin(x, Galois::MethodFlag::NONE),
                              HighDegreeIndexer::indexgraph->edge_end(y, Galois::MethodFlag::NONE));
    unsigned ry = std::distance(HighDegreeIndexer::indexgraph->edge_begin(x, Galois::MethodFlag::NONE),
...
unsigned findMatching(MetisGraph* coarseMetisGraph, bool useRM, bool use2Hop, bool verbose) {
  ...
  if(useRM) {
    ...
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::LockFreeSkipList<LowDegreeComparer, GNode>> GPQ;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::SprayList<LowDegreeNodeComparer, GNode>> SL;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::MultiQueue<LowDegreeComparer, GNode, 4>> MQ4;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::DistQueue<LowDegreeComparer, GNode, false>> PTSL;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::DistQueue<LowDegreeComparer, GNode, true>> PPSL;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::LocalPQ<LowDegreeComparer, GNode>> LPQ;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::SwarmPQ<LowDegreeComparer, GNode>> SWARMPQ;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::PartitionPQ<LowDegreeComparer, LowDegreeHasher, GNode>> PPQ;
    ...
    if (useOBIM) {
      if (wl == "obim")
        ...
        Galois::for_each_local(*fineMetisGraph->getGraph(), pHEM, Galois::loopname("match"), Galois::wl<GPQ>());
      else if (wl == "spraylist")
        Galois::for_each_local(*fineMetisGraph->getGraph(), pHEM, Galois::loopname("match"), Galois::wl<SL>());
      else if (wl == "multiqueue4")
        Galois::for_each_local(*fineMetisGraph->getGraph(), pHEM, Galois::loopname("match"), Galois::wl<MQ4>());
      else if (wl == "thrskiplist")
        Galois::for_each_local(*fineMetisGraph->getGraph(), pHEM, Galois::loopname("match"), Galois::wl<PTSL>());
      else if (wl == "pkgskiplist")
        Galois::for_each_local(*fineMetisGraph->getGraph(), pHEM, Galois::loopname("match"), Galois::wl<PPSL>());
      else if (wl == "lpq")
        Galois::for_each_local(*fineMetisGraph->getGraph(), pHEM, Galois::loopname("match"), Galois::wl<LPQ>());
      else if (wl == "swarm")
        Galois::for_each_local(*fineMetisGraph->getGraph(), pHEM, Galois::loopname("match"), Galois::wl<SWARMPQ>());
      else if (wl == "ppq")
        Galois::for_each_local(*fineMetisGraph->getGraph(), pHEM, Galois::loopname("match"), Galois::wl<PPQ>());
      else
  ...
  if (use2Hop) {
    ...
    if (useOBIM) {
      if (wl == "obim")
        ...
        Galois::for_each_local(*fineMetisGraph->getGraph(), p2HEM, Galois::loopname("match"), Galois::wl<GPQ>());
      else if (wl == "spraylist")
        Galois::for_each_local(*fineMetisGraph->getGraph(), p2HEM, Galois::loopname("match"), Galois::wl<SL>());
      else if (wl == "multiqueue4")
        Galois::for_each_local(*fineMetisGraph->getGraph(), p2HEM, Galois::loopname("match"), Galois::wl<MQ4>());
      else if (wl == "thrskiplist")
        Galois::for_each_local(*fineMetisGraph->getGraph(), p2HEM, Galois::loopname("match"), Galois::wl<PTSL>());
      else if (wl == "pkgskiplist")
        Galois::for_each_local(*fineMetisGraph->getGraph(), p2HEM, Galois::loopname("match"), Galois::wl<PPSL>());
      else if (wl == "lpq")
        Galois::for_each_local(*fineMetisGraph->getGraph(), p2HEM, Galois::loopname("match"), Galois::wl<LPQ>());
      else if (wl == "swarm")
        Galois::for_each_local(*fineMetisGraph->getGraph(), p2HEM, Galois::loopname("match"), Galois::wl<SWARMPQ>());
      else if (wl == "ppq")
        Galois::for_each_local(*fineMetisGraph->getGraph(), p2HEM, Galois::loopname("match"), Galois::wl<PPQ>());
      else
        std::cerr << "No work list!" << "\n";
    } else
