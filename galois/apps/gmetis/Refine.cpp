...
struct gainHasher : public std::unary_function<GNode, unsigned long> {

  unsigned long operator()(GNode n) {
    return (unsigned long) n;
  }
};

struct gainComparer: public std::binary_function<const GNode&, const GNode&, unsigned> {
  static GGraph* g;

  unsigned operator()(const GNode& x, const GNode& y) const {
    int rx = 0, ry = 0;
    Galois::MethodFlag flag = Galois::NONE;
    unsigned int nPart = g->getData(x, flag).getPart();
    for (auto ii = g->edge_begin(x, flag), ee = g->edge_end(x); ii != ee; ++ii) {
      GNode neigh = g->getEdgeDst(ii);
      if (g->getData(neigh, flag).getPart() == nPart)
        rx -= g->getEdgeData(ii, flag);
      else
        rx += g->getEdgeData(ii, flag);
    }
    nPart = g->getData(y, flag).getPart();
    for (auto ii = g->edge_begin(y, flag), ee = g->edge_end(y); ii != ee; ++ii) {
      GNode neigh = g->getEdgeDst(ii);
      if (g->getData(neigh, flag).getPart() == nPart)
        ry -= g->getEdgeData(ii, flag);
      else
        ry += g->getEdgeData(ii, flag);
    }
    return -rx > -ry;
...
struct gainNodeComparer: public std::binary_function<const GNode&, const GNode&, unsigned> {
  static GGraph* g;
...
GGraph* gainComparer::g;
GGraph* gainNodeComparer::g;
...
struct refine_BKL2 {
  ...
  void operator()(GNode n, Context& cnx) {
    ...
    if (curpart != newpart) {
      ...
      for (auto ii = cg.edge_begin(n), ee = cg.edge_end(n); ii != ee; ++ii, nEdges++) {
        GNode neigh = cg.getEdgeDst(ii);
  ...
  static void go(unsigned mins, unsigned maxs, GGraph& cg, GGraph* fg,  std::vector<partInfo>& p) {
    ...
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::LockFreeSkipList<gainComparer, GNode>> GPQ;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::SprayList<gainNodeComparer, GNode>> SL;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::MultiQueue<gainComparer, GNode, 4>> MQ4;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::DistQueue<gainComparer, GNode, false>> PTSL;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::DistQueue<gainComparer, GNode, true>> PPSL;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::LocalPQ<gainComparer, GNode>> LPQ;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::SwarmPQ<gainComparer, GNode>> SWARMPQ;
    typedef Galois::WorkList::GlobPQ<GNode, Galois::WorkList::PartitionPQ<gainComparer, gainHasher, GNode>> PPQ;

    gainIndexer::g = &cg;
    gainComparer::g = &cg;
    gainNodeComparer::g = &cg;
    ...
    if (wl == "obim")
      ...
      Galois::for_each_local(boundary, refine_BKL2(mins, maxs, cg, fg, p), Galois::loopname("refine"), Galois::wl<GPQ>());
    else if (wl == "spraylist")
      Galois::for_each_local(boundary, refine_BKL2(mins, maxs, cg, fg, p), Galois::loopname("refine"), Galois::wl<SL>());
    else if (wl == "multiqueue4")
      Galois::for_each_local(boundary, refine_BKL2(mins, maxs, cg, fg, p), Galois::loopname("refine"), Galois::wl<MQ4>());
    else if (wl == "thrskiplist")
      Galois::for_each_local(boundary, refine_BKL2(mins, maxs, cg, fg, p), Galois::loopname("refine"), Galois::wl<PTSL>());
    else if (wl == "pkgskiplist")
      Galois::for_each_local(boundary, refine_BKL2(mins, maxs, cg, fg, p), Galois::loopname("refine"), Galois::wl<PPSL>());
    else if (wl == "lpq")
      Galois::for_each_local(boundary, refine_BKL2(mins, maxs, cg, fg, p), Galois::loopname("refine"), Galois::wl<LPQ>());
    else if (wl == "swarm")
      Galois::for_each_local(boundary, refine_BKL2(mins, maxs, cg, fg, p), Galois::loopname("refine"), Galois::wl<SWARMPQ>());
    else if (wl == "ppq")
      Galois::for_each_local(boundary, refine_BKL2(mins, maxs, cg, fg, p), Galois::loopname("refine"), Galois::wl<PPQ>());
    else
...
struct ChangePart {//move each node to its nearest cluster
  ...
  void operator()(GNode n, Galois::UserContext<GNode>& ctx) {
    ...
    g.getData(n, Galois::MethodFlag::NONE).setPart(partition);
  }
...
struct ComputeClusterDist {
  ...
  void operator()(GNode n, Galois::UserContext<GNode>& ctx) {
    unsigned int clust = g.getData(n, Galois::MethodFlag::NONE).getPart();
    int degreet =0;
