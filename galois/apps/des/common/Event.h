/** Event: is the basic structure of an event in the simulation -*- C++ -*-
 * @file
 * @section License
 *
...
struct EventRecvTimeLocalTieBrkCmp {
  ...
  struct Comparer: public std::binary_function<const Event_tp&, const Event_tp&, unsigned> {
    unsigned operator()(const Event_tp& x, const Event_tp& y) const {
      return EventRecvTimeLocalTieBrkCmp::compare (x, y) > 0;
    }
  };
