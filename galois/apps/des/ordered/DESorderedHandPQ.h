...
#ifndef DES_ORDERED_HAND_PQ_H
#define DES_ORDERED_HAND_PQ_H
...
struct SimObjInfo: public TypeHelper {
  ...
  struct MarkedEvent {
    ...
    bool mark () const {
      ...
      if (flag == false) { 
        return __sync_bool_compare_and_swap(&flag, false, true);
      } else {
        return false;
      }
    }
    ...
    // the skiplist uses equality comparison to prevent addings multiple
    // items, but we want to allow them.
    bool operator==(const MarkedEvent& other) const {
      return false;
    }
  ...
  typedef Galois::WorkList::LockFreeSkipList<Cmp_ty::Comparer, MarkedEvent> PQ;
  ...
  void recv (const Event_ty& event) {
    ...
    pendingEvents->push (MarkedEvent(event));
  }
  ...
  MarkedEvent& getMin () const {
    MarkedEvent& ret = pendingEvents->get_min();
    return ret;
  }

  bool isMin (const Event_ty& event) const {
    Event_ty e = pendingEvents->get_min();
    return event == e;
  }


  Event_ty removeMin () {
    Event_ty event = pendingEvents->try_pop();
    return event;
  }
  ...
  bool canAddMin () const {
    while (true) {
      if (pendingEvents->empty())
        return false;
      MarkedEvent& event = pendingEvents->get_min();
      if (!(!event.isMarked() && isReady(event) && event.mark()))
        return false;
      if (&(pendingEvents->get_min()) == &event && !pendingEvents->empty())
        return true;
      event.unmark();
    }
  }
