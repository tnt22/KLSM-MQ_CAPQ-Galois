...
static cll::opt<std::string> filename("file", cll::desc("Graph to use"), cll::value_desc("file"), cll::init("inputs/random"));
...
struct MatchingABMP {
  ...
  struct Hasher: public std::unary_function<const WorkItem&,unsigned long> {
    unsigned long operator()(const WorkItem& n) const {
      return (unsigned long) n.first;
    }
  };
  ...
  bool nextEdge(G& g, const GraphNode& src, GraphNode& next, int& nEdges) {
  ...
  bool operator()(G& g, const GraphNode& root, Galois::UserContext<WorkItem>& ctx) {
    ...
    while (true) {
      ...
      if (g.getData(cur, Galois::MethodFlag::NONE).free && g.getData(cur, Galois::MethodFlag::NONE).layer == 0) {
        ...
        if (trackWork) *nOverall += nEdges;
        return false;
      } else if (nextEdge(g, cur, next, nEdges)) {
        // (2) Advance
  ...
  struct Process {
    ...
    void operator()(const WorkItem& item, Galois::UserContext<WorkItem>& ctx) {
      ...
      //  ctx.breakLoop();
      //}
      //printf("T%p node %zd pri %u\n", &ctx, g.getData(item.first, Galois::MethodFlag::NONE).id, item.second);
  ...
  void operator()(G& g) {
    ...
    NodeList A = g.A;
    if (filename == "inputs/random")
      std::random_shuffle( A.begin(), A.end() );
    for (typename NodeList::iterator ii = A.begin(), ei = A.end(); ii != ei; ++ii) {
    ...
    typedef GlobPQ<WorkItem, SprayList<Greater, WorkItem>> SL;
    ...
    if (wl == "obim")
      ...
      Galois::for_each(initial.begin(), initial.end(), Process(*this, g, maxLayer, size), Galois::wl<GPQ>());
    else if (wl == "spraylist")
      Galois::for_each(initial.begin(), initial.end(), Process(*this, g, maxLayer, size), Galois::wl<SL>());
    else if (wl == "multiqueue4")
      Galois::for_each(initial.begin(), initial.end(), Process(*this, g, maxLayer, size), Galois::wl<MQ4>());
    else if (wl == "thrskiplist")
      Galois::for_each(initial.begin(), initial.end(), Process(*this, g, maxLayer, size), Galois::wl<PTSL>());
    else if (wl == "pkgskiplist")
      Galois::for_each(initial.begin(), initial.end(), Process(*this, g, maxLayer, size), Galois::wl<PPSL>());
    else if (wl == "lpq")
      Galois::for_each(initial.begin(), initial.end(), Process(*this, g, maxLayer, size), Galois::wl<LPQ>());
    else if (wl == "swarm")
      Galois::for_each(initial.begin(), initial.end(), Process(*this, g, maxLayer, size), Galois::wl<SWARMPQ>());
    else if (wl == "ppq")
      Galois::for_each(initial.begin(), initial.end(), Process(*this, g, maxLayer, size), Galois::wl<PPQ>());
    else
...
void generateInput(int numA, int numB, int numEdges, int numGroups, G* g) {
  ...
  if (filename != "inputs/random") {
    FileGraph fg;
    typename G::NodeList all;

    fg.structureFromFile(filename);
    for (FileGraph::iterator ii = fg.begin(), ei = fg.end(); ii != ei; ++ii) {
      FileGraph::GraphNode src = *ii;
      FileGraph::edge_iterator nn = fg.edge_begin(*ii);
      FileGraph::edge_iterator en = fg.edge_end(*ii);
      typename G::GraphNode n = g->createNode(node_type(src));
      g->addNode(n, nn != en);
      all.push_back(n);
    }
    for (FileGraph::iterator ii = fg.begin(), ei = fg.end(); ii != ei; ++ii) {
      FileGraph::GraphNode src = *ii;
      for (FileGraph::edge_iterator nn = fg.edge_begin(*ii), en = fg.edge_end(*ii); nn != en; ++nn) {
        FileGraph::GraphNode dst = fg.getEdgeDst(nn);
        g->addEdge(all[src], all[dst]);
      }
    }
    return;
  }
