...
block<K, V>::block(const size_t power_of_2) :
    ...
    m_owner_tid(Galois::Runtime::LL::getTID()),
...
block<K, V>::print()
{
    for (int i = 0; i < m_last; i++) {
      auto it = m_block_items[i].m_item;
      printf("[%ld %ld, %p] ", m_block_items[i].m_key, it->val().w, it->val().n);
    }
}
                   ...
                   const size_t rhs_first)
{
    ...
    if (size > m_capacity) {
        ...
        // XXX BUG??
        // This behavior seems wrong.  In dist_lsm.insert, the code assumues
        // that a merge succeeds.
        return;
    }
...
{
    const bool called_by_owner = (Galois::Runtime::LL::getTID() == m_owner_tid);
