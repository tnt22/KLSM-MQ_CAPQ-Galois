 ...
 */
#ifndef GALOIS_WORKLIST_SL_DOBIM_H
#define GALOIS_WORKLIST_SL_DOBIM_H
...
template<class Indexer = DummyIndexer<int>, typename Container = FIFO<>,
  ...
  typename T=int,
  typename Index=int>
struct dSkipListOrderedByIntegerMetric : private boost::noncopyable {
  template<bool _concurrent>
  struct rethread { typedef dSkipListOrderedByIntegerMetric<Indexer, typename Container::template rethread<_concurrent>::type, BlockPeriod, T, Index> type; };

  template<typename _T>
  struct retype { typedef dSkipListOrderedByIntegerMetric<Indexer, typename Container::template retype<_T>::type, BlockPeriod, _T, typename std::result_of<Indexer(_T)>::type> type; };

  template<unsigned _period>
  struct with_block_period { typedef dSkipListOrderedByIntegerMetric<Indexer, Container, _period, T, Index> type; };

  template<typename _container>
  struct with_container { typedef dSkipListOrderedByIntegerMetric<Indexer, _container, BlockPeriod, T, Index> type; };

  template<typename _indexer>
  struct with_indexer { typedef dSkipListOrderedByIntegerMetric<_indexer, Container, BlockPeriod, T, Index> type; };
...
public:
  dSkipListOrderedByIntegerMetric(const Indexer& x = Indexer()): heap(sizeof(CTy)), indexer(x), nQ(Galois::getActiveThreads()) {
  ...
  ~dSkipListOrderedByIntegerMetric() {
    sl_node_t *ii, *next;
...
GALOIS_WLCOMPILECHECK(dSkipListOrderedByIntegerMetric)
