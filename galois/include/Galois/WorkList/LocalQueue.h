...
private:
  ...
  unsigned int pushGlobal(const RangeTy& range, typename std::enable_if<Enable>::type* = 0) {
    auto rp = range.local_pair();
    return local.getLocal()->push(rp.first, rp.second);
  }

  template<typename RangeTy, bool Enable = std::is_same<Global,NoGlobalQueue<T> >::value>
  unsigned int pushGlobal(const RangeTy& range, typename std::enable_if<!Enable>::type* = 0) {
    return global.push_initial(range);
  }
...
public:
  ...
  unsigned int push(Iter b, Iter e) {
    return local.getLocal()->push(b,e);
  }

  template<typename RangeTy>
  unsigned int push_initial(const RangeTy& range) {
    return pushGlobal(range);
  }
